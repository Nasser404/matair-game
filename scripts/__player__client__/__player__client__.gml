function player_client() constructor {
    ...
    function packet_received(_network_event) {
        ...
        switch (_type) {
            ...
            case network_type_data :
                ...
                self.handle_packet(_ip, _port, _socket, _buffer);
            break;
    ...
    /// @desc Handle packets received
    /// @param {string} ip 
    /// @param {Int} port
    /// @param {Id.socket} socket
    /// @param {string} data
    function handle_packet(_ip, _port, _socket, _buffer) {
        ...
        switch (_type) {
            case MESSAGE_TYPE.IDENTIFICATION   : handle_identification    (_socket, _data);     break;
            case MESSAGE_TYPE.PING             : handle_ping              (_socket, _data);     break;
            case MESSAGE_TYPE.PLAYER_CONNECT   : handle_player_connect    (_socket, _data);     break;
            case MESSAGE_TYPE.VIEWER_CONNECT   : handle_viewer_connect    (_socket, _data);     break; 
            case MESSAGE_TYPE.ORB_DATA         : handle_orb_data          (_socket, _data);     break;
            case MESSAGE_TYPE.ORB_LIST         : handle_orb_list          (_socket, _data);     break;
            case MESSAGE_TYPE.GAME_LIST        : handle_game_list         (_socket, _data);     break
            case MESSAGE_TYPE.GAME_DATA        : handle_game_data         (_socket, _data);     break;
            case MESSAGE_TYPE.MOVE             : handle_network_move      (_socket, _data);     break; 
            case MESSAGE_TYPE.GAME_INFO        : handle_game_info         (_socket, _data);     break;
            case MESSAGE_TYPE.DISCONNECT_REASON: handle_disconnect_reason (_socket, _data);     break
            case MESSAGE_TYPE.GAME_CHAT        : handle_game_chat         (_socket, _data);     break;   
            case MESSAGE_TYPE.INFORMATION      : handle_server_information(_socket, _data);     break;
    ...
    function handle_game_data(_socket,_data) {
          if (global.client_type == CLIENT_TYPE.PLAYER) handle_game_data_player(_socket, _data); 
    ...
    function handle_identification(_socket, _data) {
        ...
        _data = {"type" : MESSAGE_TYPE.IDENTIFICATION, "identifier": CLIENT_TYPE.PLAYER};
    ...
    function handle_ping(_socket, _data) {
    ...
    function handle_game_chat(_socket, _data) {
    ...
    function handle_game_data_player(_socket, _data){
        ...
        if (!global.in_game) {
            ...
            global.game_board.init_all_moves(global.color)
            global.in_game = true;
        } else {
    ...
    function handle_network_move(_socket, _data) {
        var _from_pos = _data[$ "from"];
        var _to_pos   = _data[$ "to"];
        if (global.game_board != undefined) {
            global.game_board.move_piece(_from_pos, _to_pos);
            if (global.in_game) global.asked_a_move = false;
        }
